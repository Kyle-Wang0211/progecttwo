# PR1 Constitution Patch: Cursor Implementation Prompt (V2 - Enhanced)
...
|----|----|---------|----- |------|----------|--------|--------|------|
| D001 | PR#1 | MAX_FRAMES | 2000 | 5000 | RELAXED | 15-min video needs more frames | Local | 2026-01-28 |
| D002 | PR#1 | SFM_REGISTRATION_MIN | 0.60 | 0.75 | STRICTER | Quality guarantee | Local | 2026-01-28 |
| ... | ... | ... | ... | ... | ... | ... | ... | ... |
...
4. `docs/drift/` directory (create if not exists)
5. `docs/drift/DRIFT_REGISTRY.md`
6. Update `docs/constitution/INDEX.md` (add all 3 new entries)
...
# Verify no syntax errors in markdown (optional)
...
### 4.3 CI Validation (Post-Implementation)
...
### 5.1 If Implementation Fails

```bash
# Restore constitution directory
git checkout -- docs/constitution/

# Remove drift directory if newly created
rm -rf docs/drift/
...
git checkout -- docs/constitution/CI_HARDENING_CONSTITUTION.md

# Or remove if newly created
rm docs/constitution/CI_HARDENING_CONSTITUTION.md
```
...
grep -E "CI_HARDENING|MODULE_CONTRACT|SPEC_DRIFT" docs/constitution/INDEX.md
# Should return 0 results after full rollback
```
...
### 6.1 Hash Files (Recommended for Self-Compliance)

To make new constitution documents comply with MODULE_CONTRACT_EQUIVALENCE §2.4:

```bash
# Generate hash files after document creation
...
### 6.2 Additional Files (If Hash Files Created)

Add to file creation list:
...
### 7.1 Why This Design Is Future-Proof
...
│  • grep -c "CI_HARDENING" docs/constitution/INDEX.md                │
│  • swift test --filter CaptureStaticScanTests                       │
│  • swift build                                                      │
│                                                                     │
│  ROLLBACK:                                                          │
