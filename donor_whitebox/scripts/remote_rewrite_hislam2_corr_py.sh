...
FILE="${ROOT}/hislam2/modules/corr.py"
...
class CorrBlock:
    ...
    def corr(fmap1, fmap2):
        ...
        # RTX 5090 + torch/cu128 can throw invalid cublas values on the original
        # autocast half-precision batched GEMM. Force a contiguous fp32 path here.
        with torch.amp.autocast("cuda", enabled=False):
            lhs = fmap1.transpose(1, 2).contiguous().float()
            rhs = fmap2.contiguous().float()
            corr = torch.stack([lhs[i] @ rhs[i] for i in range(lhs.shape[0])], dim=0)
...
class CorrLayer(torch.autograd.Function):
    ...
    def backward(ctx, grad_corr):
        ...
        grads = droid_backends.altcorr_backward(fmap1, fmap2, coords, grad_corr, ii, jj, ctx.r)
        fmap1_grad, fmap2_grad, coords_grad = grads[:3]
        return fmap1_grad, fmap2_grad, coords_grad, None, None, None
