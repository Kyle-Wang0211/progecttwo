...
class JobContext:
    ...
    def from_assignment(cls, assignment: dict[str, Any], *, worker_id: str) -> "JobContext":
        ...
        hq_enabled = str(
            profile.get("hq_refine") or "graphdeco_3dgs"
        ).lower() not in {"off", "none", "disabled", "false"}
        mesh_export_enabled = str(
            profile.get("optional_mesh_export")
            or ("enabled" if config.optional_mesh_export_enabled_default else "disabled")
        ).lower() not in {"off", "none", "disabled", "false"}
        return cls(
            job_id=job_id,
            worker_id=worker_id,
            ...
            should_run_optional_mesh_export=mesh_export_enabled,
