...
class MatchaAttemptPlan:
    attempt_index: int
    reason: str
    working_point_cap: int


def run_matcha_mesh(ctx: JobContext, *, progress_callback=None) -> None:
    ctx.current_stage = "matcha_mesh_extract"
    _run_matcha_mesh_subprocess(ctx, progress_callback=progress_callback)


def _run_matcha_mesh_subprocess(
    ctx: JobContext,
    *,
    progress_callback: Callable[[dict[str, Any]], None] | None = None,
) -> None:
    assert ctx.matcha_dir is not None
    progress_path = ctx.matcha_dir / "matcha_mesh_extract.progress.json"
    log_path = ctx.matcha_dir / "matcha_mesh_extract.log"
    runtime_path = ctx.matcha_dir / "matcha_mesh_extract.runtime.json"
    ...
    attempt_plans = _build_matcha_attempt_plans()
    ...
    for attempt_plan in attempt_plans:
        ...
        attempt_result, last_progress_signature = _run_single_matcha_attempt(
            ctx,
            attempt_plan=attempt_plan,
        ...
        if _should_retry_matcha_attempt(attempt_result) and attempt_plan != attempt_plans[-1]:
            continue

        last_error = _format_matcha_failure(attempt_result, log_path=log_path)
        break
    ...
    _write_json(runtime_path, runtime_payload)
    raise RuntimeError(last_error or f"matcha_mesh_extract_failed:log={log_path}")


def _build_matcha_attempt_plans() -> list[MatchaAttemptPlan]:
    plans = [
        MatchaAttemptPlan(
            attempt_index=0,
            reason="initial_budget",
            working_point_cap=max(1, int(config.matcha_working_point_cap)),
        )
    ]

    max_retries = max(0, int(config.matcha_max_retries))
    for _ in range(max_retries):
        previous = plans[-1]
        next_point_cap = max(
            int(config.matcha_retry_point_cap_floor),
            int(round(previous.working_point_cap * float(config.matcha_retry_point_cap_scale))),
        )
        if next_point_cap == previous.working_point_cap:
            break
        plans.append(
            MatchaAttemptPlan(
                attempt_index=len(plans),
                reason="retry_tighter_working_checkpoint",
                working_point_cap=int(next_point_cap),
            )
        )
    return plans
...
def _emit_retry_progress(
    *,
    ...
    attempt_plan: MatchaAttemptPlan,
    total_attempts: int,
) -> None:
    ...
    progress_callback(
        {
            "progress": 0.02,
            "title": f"正在收紧 HQ 网格提取预算并重试 {attempt_plan.attempt_index + 1}/{total_attempts}",
            "detail": "上一次 MAtCha 提取被系统提前终止，这次会先构建更小的 working checkpoint 再继续提取。",
            "metrics": {
                "matcha_phase": "retry_with_tighter_budget",
                "matcha_attempt_index": str(attempt_plan.attempt_index),
                "matcha_total_attempts": str(total_attempts),
                "matcha_working_point_cap": str(attempt_plan.working_point_cap),
            },
        }
    )


def _run_single_matcha_attempt(
    ctx: JobContext,
    *,
    attempt_plan: MatchaAttemptPlan,
    progress_path: Path,
    ...
    command = [
        sys.executable,
        "-m",
        "worker_object_slam3r_surface_v1.pipeline.run_matcha_mesh",
        "--child",
        "--job-id",
        ctx.job_id,
        "--progress-path",
    ...
    env.update(
        {
            "OBJECT_SLAM3R_SURFACE_MATCHA_ATTEMPT_INDEX": str(attempt_plan.attempt_index),
            "OBJECT_SLAM3R_SURFACE_MATCHA_WORKING_POINT_CAP": str(attempt_plan.working_point_cap),
        }
    )
    ...
    with log_path.open("a", encoding="utf-8") as log_file:
        log_file.write(
            ...
            f"working_point_cap={attempt_plan.working_point_cap}\n"
        )
        log_file.flush()
        ...
        while True:
            ...
            if return_code is not None:
                break
            if time.time() - start_time > float(config.matcha_stage_timeout_sec):
                timed_out = True
                process.terminate()
                try:
    ...
    return (
        {
            ...
            "working_point_cap": int(attempt_plan.working_point_cap),
            "pid": int(process.pid),
            "status": status,
...
def _should_retry_matcha_attempt(attempt_result: dict[str, Any]) -> bool:
...
def _format_matcha_failure(attempt_result: dict[str, Any], *, log_path: Path) -> str:
    ...
    if bool(attempt_result.get("timed_out")):
        return (
            "matcha_mesh_extract_timeout:"
            f"timeout_sec={int(config.matcha_stage_timeout_sec)}:"
    ...
    if isinstance(exit_signal, int) and exit_signal > 0:
        return (
            "matcha_mesh_extract_subprocess_failed:"
...
def _run_child(job_id: str, progress_path: Path, attempt_index: int) -> int:
    ...
    ctx.current_stage = "matcha_mesh_extract"
    print(
        f"[matcha_mesh_extract] child_start job_id={job_id} attempt_index={attempt_index}",
        flush=True,
    )
    _write_json(
        progress_path,
        {
            "progress": 0.02,
            "title": "正在准备 MAtCha 网格提取",
            "detail": "正在读取高斯检查点并准备 tetra mesh 提取。",
            "metrics": {
                "matcha_phase": "child_start",
                "matcha_attempt_index": str(attempt_index),
            },
        },
    )
    try:
        run_matcha_mesh_extraction(
            ctx,
    ...
    print(f"[matcha_mesh_extract] child_done job_id={job_id}", flush=True)
    return 0


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run matcha_mesh_extract in a subprocess-safe mode.")
    parser.add_argument("--child", action="store_true", help="Run the MAtCha child worker inline.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Retry attempt index used for progress metrics and working checkpoint naming.",
    )
